...
'(define 5)
;expect (define 5)
(define x (begin (display 21) (+ 15 18)))
;expect 21x
x
;expect 33
(define (function_create x y) 
  (define (new_function z)
    (if (x y) z  'False!)
  )
)
...
(cond ((= (+ 2 2) 4) 'thisstatementisaccurate
 (+ 68 2))
 (else (/ 1 0) 0))
; expect 70

((function_create 
  (define (true_or_false q)
  (cond
    ((and q 3) #t)
    (else #f)
  )
...
(a (+ 9 5 4 3 2 1 (* 0 5)))
;expect 27
(define z (+ 100 200 300 500))
;expect z
z
;expect 1100
(define x (lambda (x) (* 10 x)))
(let ((x 42)
           (y (x z)))  
       (list x y))
;expect (42 11000)
x
;expect (lambda (x) (* 10 x))
(define-macro (function expr)
        (list (list 'begin (list display expr) (list display expr)))
) 
;expect function
(function 'Hellothisismytestforamacroexpression)
;expect Error
(define (print-and-cube x)
  (print x)
  (* x x x))
;expect print-and-cube
(print-and-cube 5)
; expect 5 ; 125
(y 1 2)
; expect Error
...
`(1 2 3 ,4 5)
;expect (1 2 3 4 5)
(begin (+ 2 5 3) (+ 5 6) (* 5 6) (+ 1 2 3 4 5) (- (+ 1 2 3) (* 2 3)))
;expect 0
(eval_all ())
;expect Error
(and (< 5 6) (< 2 3))
