...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        child = Frame(self)
        if len(vals) > len(formals) or len(formals) > len(vals):
            raise SchemeError('Too few or too many values were given for the given formals!')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        expression = Pair(target.second, expressions.second)
        env.define(target.first , do_lambda_form(expression, env))
        return target.first
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    bindings.map(lambda binding: check_form(binding, 2, 2))


    check_formals(bindings.map(lambda binding: binding.first))

    

    return_frame =  env.make_child_frame(bindings.map(lambda binding: binding.first), bindings.map(lambda binding: scheme_eval(binding.second.first, env) ))
    return return_frame
...
def do_define_macro(expressions, env):
    ...
    if isinstance(value, Pair):
        if scheme_symbolp(value.first):
            check_formals(value.second)
            env.define(value.first, MacroProcedure(value.second, expressions.second, env))
            return value.first
    raise SchemeError("This Macro Definition will not work!")
    # END Problem 21
